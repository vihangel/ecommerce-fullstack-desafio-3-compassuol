...
let NewsletterController = class NewsletterController {
    ...
    async sendNewsletter(body) {
        try {
            await this.newsletterService.sendNewsletterEmail(body.to);
            return { message: 'Email sent successfully' };
        }
        catch (error) {
            console.error('Error in sendNewsletter:', error.message);
            throw new common_1.HttpException({ error: 'Could not send email', details: error.message }, common_1.HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
};
